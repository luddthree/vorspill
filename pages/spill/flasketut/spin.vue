...
onMounted(() => {
  ...
  const finalSpinDuration = 2; // Duration of the final spin in seconds
  ...
  spinStyle.value = {
    animation: `spin ${fastSpinDuration}s forwards, final-spin ${finalSpinDuration}s ease-out forwards`,
    animationDelay: `0s, ${fastSpinDuration}s`,
  };

  // Correctly use setTimeout to update spinCompleted after the animation ends
  setTimeout(() => {
    spinCompleted.value = true; // This should only happen once the animation is fully complete
  }, (fastSpinDuration + finalSpinDuration) * 1000); // Calculate total duration correctly
